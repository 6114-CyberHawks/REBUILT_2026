...
public class AutonomousRight extends Command {
  ...
  public AutonomousRight(DriveSubsystem s_DriveSubsystem, AlmostDataManager s_DataTableManager,
      HoodSubsystem s_HoodSubsystem, TurretSubsystem s_TurretSubsystem, IntakeSubsystem s_IntakeSubsystem,
      HopperSubsystem s_HopperSubsystem) {
    timer = new Timer();
    driveSubsystem = s_DriveSubsystem;
    dataTableManager = s_DataTableManager;
    hoodSubsystem = s_HoodSubsystem;
    turretSubsystem = s_TurretSubsystem;
  ...
  // SmartDashboard.putBoolean("Is Code active?", y);
  // SmartDashboard.putBoolean("Is feed Going", x);
  // }
  ...
  public void execute() {
    ...
    if (timer.get() < 5 && timer.get() > 0) {
      ...
      hoodSubsystem.setLowPosition();
      turretSubsystem.ShootFuel();
      intakeSubsystem.deployIntake();
      turretSubsystem.DelayMS(2000);
      intakeSubsystem.StopIntakePivot();
      turretSubsystem.FeedForward();
      turretSubsystem.DelayMS(2000);
      hopperSubsystem.Forward();
      turretSubsystem.DelayMS(5000);
      turretSubsystem.StopFeed();
